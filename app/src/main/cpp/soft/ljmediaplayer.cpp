...
LJMediaPlayer* LJMediaPlayer::Create() {
    LJMediaPlayer* mp=new LJMediaPlayer();
    return mp;
}

void LJMediaPlayer::Destroy(LJMediaPlayer *mp) {
    if(mp){
        delete mp;
    }
}
...
LJMediaPlayer::LJMediaPlayer() {
    ...
    this->jvm = 0;
    this->buffer_full = 0;
    this->state = LJPLAYER_STATE_ERROR;
...
int LJMediaPlayer::setup(JavaVM *jvm, jobject thiz, jobject weak_this) {
    ...
    LJMediaPlayerListener* listener = new LJMediaPlayerListener(jvm, thiz, weak_this);
    if (listener == NULL) {
        av_log(NULL,AV_LOG_ERROR, "%s: new JNIMediaPlayerListener fail\n",__FUNCTION__);
        return LJPLAYER_ERROR_NO_MEMORY;
    }
    this->mediaplayer_listener  = listener;

    this->state = LJPLAYER_STATE_IDLE;

    av_log(NULL, RELEASE_DEBUG_LOG_LEVEL, "%s: out\n",__FUNCTION__);
    return LJPLAYER_SUCCESS;
}

int LJMediaPlayer::set_surface(JavaVM *jvm, jobject jsurface) { return LJPLAYER_SUCCESS;}

int LJMediaPlayer::set_video_scaling_mode(int mode) {
...
int LJMediaPlayer::set_datasource(const char *path) {
    av_log(NULL, AV_LOG_DEBUG, "%s: into\n",__FUNCTION__);

    if(!path){
        av_log(NULL, AV_LOG_ERROR, "%s: path NULL\n",__FUNCTION__);
        return LJPLAYER_ERROR_PARAMETER;
    }

    if(this->state != LJPLAYER_STATE_IDLE){
        av_log(NULL, AV_LOG_ERROR, "%s: wrong state\n",__FUNCTION__);
        return LJPLAYER_ERROR_LJPLAYER_STATE;
    }
...
int LJMediaPlayer::set_looping(int loop) {
    av_log(NULL, AV_LOG_DEBUG, "%s: into\n",__FUNCTION__);
...
int LJMediaPlayer::prepare() {
    int ret;

    av_log(NULL, AV_LOG_DEBUG, "%s: into\n",__FUNCTION__);

    if(	this->state != LJPLAYER_STATE_INIT &&
           this->state != LJPLAYER_STATE_PREPARED &&
           this->state != LJPLAYER_STATE_STOPPED){
        av_log(NULL, AV_LOG_ERROR, "%s: wrong state,is->state=%d\n",__FUNCTION__,this->state);
        return LJPLAYER_ERROR_LJPLAYER_STATE;
    }

    if(this->state == LJPLAYER_STATE_PREPARED ){
    ...
    ////add here
    ///�����ε�����Ű�ť
    ///event thread, handle event and show video ��BUG��Ӧ���ȴ���?
    if(this->hardware_context==NULL){
        this->hardware_context = new SDL_Aout();
        this->hardware_context->mtx = NULL;
        this->hardware_context->android_opensles = NULL;
    }
    this->event_thread_id = SDL_CreateThread(event_thread, this);
    if(this->event_thread_id == 0){
        av_log(NULL, AV_LOG_FATAL, "%s create event thread fail\n",__FUNCTION__);
        return LJPLAYER_ERROR_INTERNAL;
    }else{
...
int LJMediaPlayer::prepareAsync() {
    int ret;

    av_log(NULL, RELEASE_DEBUG_LOG_LEVEL, "%s: into\n",__FUNCTION__);

    if( this->state != LJPLAYER_STATE_INIT &&
        this->state != LJPLAYER_STATE_PREPARING &&
        this->state != LJPLAYER_STATE_STOPPED){
        av_log(NULL, AV_LOG_ERROR, "%s: wrong state,is->state=%d\n",__FUNCTION__,this->state);
        return LJPLAYER_ERROR_LJPLAYER_STATE;
    }

    if(this->state == LJPLAYER_STATE_PREPARING ){
        av_log(NULL, AV_LOG_WARNING, "%s: already preparing\n",__FUNCTION__);
        return LJPLAYER_SUCCESS;
    }

    ljplayer_state last_state = this->state;
    this->state = LJPLAYER_STATE_PREPARING;
    ...
    this->stream_open_thread_id = SDL_CreateThread(stream_open_thread, this);
...
int LJMediaPlayer::start() {
    int ret ;
    av_log(NULL, AV_LOG_DEBUG, "%s: into\n",__FUNCTION__);

    if(	this->state != LJPLAYER_STATE_PREPARED &&
           this->state != LJPLAYER_STATE_PAUSED &&
           this->state != LJPLAYER_STATE_COMPLETED &&
           this->state != LJPLAYER_STATE_STARTED ){
        av_log(NULL, AV_LOG_ERROR, "%s: wrong state,is->state=%d\n",__FUNCTION__,this->state);
        return LJPLAYER_ERROR_LJPLAYER_STATE;
    }
    if(this->state == LJPLAYER_STATE_STARTED){
        av_log(NULL, AV_LOG_WARNING, "%s: already start\n",__FUNCTION__);
        return LJPLAYER_SUCCESS;
    }

    if( this->state == LJPLAYER_STATE_PREPARED ){
        av_log(NULL, AV_LOG_WARNING, "%s: on LJPLAYER_STATE_PREPARED state\n",__FUNCTION__);
        ret = stream_start();
        if( ret < 0 ){
            return LJPLAYER_ERROR_NORMAL;
        }
    }else if( this->state == LJPLAYER_STATE_PAUSED ){
        av_log(NULL, AV_LOG_WARNING, "%s: on LJPLAYER_STATE_PAUSED state\n",__FUNCTION__);
        if(this->seeking){
            av_log(NULL, AV_LOG_ERROR, "%s: seeking, just return\n",__FUNCTION__);
            return LJPLAYER_SUCCESS;
        }
        AVMessage msg;
        msg.type = SDL_PAUSE_EVENT;
        msg.data = NULL;
        msg_queue_put_private(this->messageQueue,&msg);
    }else if( this->state == LJPLAYER_STATE_COMPLETED ){
        av_log(NULL, AV_LOG_WARNING, "%s: on LJPLAYER_STATE_COMPLETED state\n",__FUNCTION__);
        AVMessage msg;
        //this->seeking = 1;
        this->send_seek_complete_notify = 0;
        this->seek_msec  = 0;
        msg.type = SDL_SEEK_EVENT;
        msg.data = (void*)&this->seek_msec; // seek_frac;
        msg_queue_put_private(this->messageQueue,&msg);
    }else{
...
int LJMediaPlayer::pause() {
    int ret ;
    av_log(NULL, AV_LOG_DEBUG, "%s: into\n",__FUNCTION__);

    if( this->state != LJPLAYER_STATE_PAUSED &&
        this->state != LJPLAYER_STATE_STARTED ){
        av_log(NULL, AV_LOG_ERROR, "%s: wrong state,is->state=%d\n",__FUNCTION__,this->state);
        return LJPLAYER_ERROR_LJPLAYER_STATE;
    }
...
int LJMediaPlayer::stop() {
    ...
    if(this->state == LJPLAYER_STATE_STOPPED){
    ...
    AVMessage event;
    event.type = SDL_RESET_EVENT;
    event.data = NULL;

    msg_queue_put_private(this->messageQueue,&event);

    SDL_WaitThread(this->event_thread_id,NULL);
...
int LJMediaPlayer::reset() {
    av_log(NULL, AV_LOG_DEBUG, "%s: into\n",__FUNCTION__);

    if( this->state == LJPLAYER_STATE_END){
        av_log(NULL, AV_LOG_ERROR, "%s: wrong state\n",__FUNCTION__);
        return LJPLAYER_ERROR_LJPLAYER_STATE;
    }
...
int LJMediaPlayer::seekto(int msec) {
    ...
    if(this->state==LJPLAYER_STATE_PREPARED){
        if(0 == this->read_tid){
            this->read_tid = SDL_CreateThread(read_thread, this);
            if (0 == this->read_tid) {
    ...
    event.type = SDL_SEEK_EVENT;
    event.data = (void*)&this->seek_msec; // seek_frac;

    msg_queue_put_private(this->messageQueue,&event);

    if(this->state==LJPLAYER_STATE_COMPLETED){
...
int LJMediaPlayer::is_playing() {
    return (this->state==LJPLAYER_STATE_STARTED);
}

int LJMediaPlayer::get_video_width() {
    return this->frame_width;
}

int LJMediaPlayer::get_video_height() {
    return this->frame_height;
}

float LJMediaPlayer::get_video_aspect_ratio() {
...
int LJMediaPlayer::get_current_position() {
    int msec = 0;
...
int LJMediaPlayer::get_duration() {
...
int LJMediaPlayer::is_looping() {
    return this->looping;
}

int LJMediaPlayer::is_buffering() {
    return !this->buffer_full;
}

int LJMediaPlayer::config(const char *cfg, int val) {
...
int LJMediaPlayer::setVolume(float left, float right) {
    ...
    event.type = SDL_SET_VOLUME_EVENT;
    event.data = (void*)vol;
...
void LJMediaPlayer::notify(int msg, int ext1, int ext2, void *obj) {
    switch (msg) {
        ...
        case MEDIA_PLAYBACK_COMPLETE:
            if(this->state == LJPLAYER_STATE_COMPLETED){
                av_log(NULL, AV_LOG_DEBUG,"%s: already in LJPLAYER_STATE_COMPLETED state",__FUNCTION__);
                return;
            }
    ...
    if (this->mediaplayer_listener != 0) {
        SDL_LockMutex(this->mp_listener_lock);
        this->mediaplayer_listener->notify(msg, ext1, ext2, obj);
        SDL_UnlockMutex(this->mp_listener_lock);
    }
...
// *************** private *******************
int LJMediaPlayer::packet_queue_put_private(PacketQueue *q, AVPacket *pkt) {
    MyAVPacketList *pkt1;
    ...
    SDL_SignalCond(q->cond);
    return LJPLAYER_SUCCESS;
}

int LJMediaPlayer::packet_queue_put(PacketQueue *q, AVPacket *pkt) {
    int ret;
...
int LJMediaPlayer::packet_queue_put_nullpacket(PacketQueue *q, int stream_index) {
...
void LJMediaPlayer::packet_queue_init(PacketQueue *q) {
...
void LJMediaPlayer::packet_queue_flush(PacketQueue *q) {
...
void LJMediaPlayer::packet_queue_flush_half(PacketQueue *q) {
    MyAVPacketList *pkt1;
    int i;
...
void LJMediaPlayer::packet_queue_destroy(PacketQueue *q) {
    packet_queue_flush(q);
    SDL_DestroyMutex(q->mutex);
    SDL_DestroyCond(q->cond);
}

void LJMediaPlayer::packet_queue_abort(PacketQueue *q) {
    SDL_LockMutex(q->mutex);
...
void LJMediaPlayer::packet_queue_start(PacketQueue *q) {
    SDL_LockMutex(q->mutex);
    q->abort_request = 0;
...
int LJMediaPlayer::packet_queue_get(PacketQueue *q, AVPacket *pkt, int block, int *serial) {
    MyAVPacketList *pkt1;
    int ret = 0;
...
void LJMediaPlayer::blend_subrect(AVPicture *dst, const AVSubtitleRect *rect, int imgw, int imgh) {
...
void LJMediaPlayer::free_picture(VideoPicture *vp) {
    if (vp->bmp) {
...
void LJMediaPlayer::free_subpicture(SubPicture *sp) {
    avsubtitle_free(&sp->sub);
}

void LJMediaPlayer::video_image_display() {
    VideoPicture *vp;
    SubPicture *sp;
    AVPicture pict;
//    SDL_Rect rect;
    ...
    if (vp->bmp) {
        ...
        SDL_DisplayYUVOverlay(vp->bmp);
    }
}

void LJMediaPlayer::video_display() {
    int ret = 0;
    if (this->video_st)
        video_image_display();
}

void LJMediaPlayer::video_redisplay() {
    VideoPicture *vp;
    SubPicture *sp;
    AVPicture pict;
    int i,ret;
...
//        SDL_ReDisplayYUVOverlay(vp->bmp);

    }
}

void LJMediaPlayer::stream_close() {
    av_log(NULL, RELEASE_DEBUG_LOG_LEVEL, "%s: into\n",__FUNCTION__);
    ...
    SDL_DestroyMutex(this->pictq_mutex);
    this->pictq_mutex = NULL;
    SDL_DestroyCond(this->pictq_cond);
    this->pictq_cond = NULL;
    SDL_DestroyMutex(this->subpq_mutex);
    this->subpq_mutex = NULL;
    SDL_DestroyCond(this->subpq_cond);
    this->subpq_cond = NULL;
    ...
    /*if(this->screen){
        ...
        SDL_CloseSurface(this->screen);  ///new add here
        this->screen = 0;
    }*/
    if(this->filename){
    ...
    if(NULL!=this->hardware_context && NULL!=this->hardware_context->mtx){
        SDL_DestroyMutex(this->hardware_context->mtx);
        this->hardware_context->mtx = NULL;
    }
    ...
    SDL_DestroyMutex(this->mp_listener_lock);
    this->mp_listener_lock = NULL;

    //SDL_DestroyMutex(this->pause_lock);
...
void LJMediaPlayer::do_exit(int destroy_vs) {
    av_log(NULL, RELEASE_DEBUG_LOG_LEVEL, "%s: into\n",__FUNCTION__);
...
int LJMediaPlayer::video_open(int force_set_video_mode, VideoPicture *vp) {

    /*if(NULL==this->screen){
        *//*
        if(vp && vp->width)
            screen = SDL_OpenSurface(vp->width, vp->vp->height);
        else
            screen = SDL_OpenSurface(-1, -1);
            *//*
        av_log(NULL, AV_LOG_INFO, "%s: SDL_OpenSurface begin\n",__FUNCTION__);
        this->screen =  SDL_OpenSurface(-1, -1, this->opengl2_render_listener );
        av_log(NULL, AV_LOG_INFO, "%s: SDL_OpenSurface end\n",__FUNCTION__);
        if(!this->screen){
            return -1;
        }
    }*/
...
double LJMediaPlayer::get_clock(Clock *c) {
...
void LJMediaPlayer::set_clock_at(Clock *c, double pts, int serial, double time) {
    c->pts = pts;
    c->last_updated = time;
...
void LJMediaPlayer::set_clock(Clock *c, double pts, int serial) {
    double time = av_gettime() / 1000000.0;
    set_clock_at(c, pts, serial, time);
}

void LJMediaPlayer::set_clock_speed(Clock *c, double speed) {
    set_clock(c, get_clock(c), c->serial);
    c->speed = speed;
}

void LJMediaPlayer::init_clock(Clock *c, int *queue_serial) {
    c->speed = 1.0;
    c->paused = 0;
...
void LJMediaPlayer::sync_clock_to_slave(Clock *c, Clock *slave) {
...
int LJMediaPlayer::get_master_sync_type() {
...
double LJMediaPlayer::get_master_clock() {
    double val;
...
void LJMediaPlayer::stream_seek(int64_t pos, int64_t rel, int seek_by_bytes) {
    if (!this->seek_req) {
        this->seek_pos = pos;
        this->seek_rel = rel;
        ...
        if (seek_by_bytes)
            this->seek_flags |= AVSEEK_FLAG_BYTE;
        this->seek_req = 1;
        SDL_SignalCond(this->continue_read_thread);
    }
}

void LJMediaPlayer::stream_toggle_pause(int unpause) {
    if (this->paused) {
...
void LJMediaPlayer::toggle_pause() {
    stream_toggle_pause(0);
    this->step = 0;
}

void LJMediaPlayer::step_to_next_frame() {
    stream_toggle_pause(1);
    this->step = 1;
}

double LJMediaPlayer::compute_target_delay(double delay) {
...
void LJMediaPlayer::pictq_next_picture() {
    /* update queue size and signal for next picture */
    if (++this->pictq_rindex == VIDEO_PICTURE_QUEUE_SIZE)
        this->pictq_rindex = 0;

    SDL_LockMutex(this->pictq_mutex);
    this->pictq_size--;
    SDL_SignalCond(this->pictq_cond);
    SDL_UnlockMutex(this->pictq_mutex);
}

int LJMediaPlayer::pictq_prev_picture() {
    VideoPicture *prevvp;
    int ret = 0;
...
void LJMediaPlayer::update_video_pts(double pts, int64_t pos, int serial) {
...
void LJMediaPlayer::video_refresh(double *remaining_time) {
    ...
    if (this->video_st) {
        ...
        if (this->pictq_size == 0) {
			...
			*/
            ...
            if (vp->serial != this->videoq.serial) { ///seek����ʾ��ǰ��ͼƬ
            ...
            if (!isnan(last_duration) && last_duration > 0 && last_duration < this->max_frame_duration) {
                /* if duration of the last frame was sane,��־������ update last_duration in MediaPlayer */
            ...
            if (this->subtitle_st) {
                while (this->subpq_size > 0) {
                        ...
                        || (sp2 && this->vidclk.pts > (sp2->pts + ((float) sp2->sub.start_display_time / 1000))))
                    {
                        ...
                        SDL_LockMutex(this->subpq_mutex);
                        this->subpq_size--;
                        SDL_SignalCond(this->subpq_cond);
                        SDL_UnlockMutex(this->subpq_mutex);
                    } else {
                        break;
                    }
                }
...
int LJMediaPlayer::alloc_picture() {
    VideoPicture *vp;
    int64_t bufferdiff;
...
int LJMediaPlayer::queue_picture(AVFrame *src_frame, double pts, int64_t pos, int serial) {
    VideoPicture *vp;
...
///#endif
    ...
    while (this->pictq_size >= VIDEO_PICTURE_QUEUE_SIZE - 1 &&
        ...
        SDL_WaitCond(this->pictq_cond, this->pictq_mutex);
    }
    SDL_UnlockMutex(this->pictq_mutex);
    ...
    if (!vp->bmp || vp->reallocate || !vp->allocated ||
        ...
        /* the allocation must be done in the main thread to avoid
           locking problems. */
        event.type = SDL_ALLOC_EVENT;
        event.data = this; ///event.user.data1 = is;

        ///av_log(NULL, AV_LOG_INFO, "%s: msg_queue_put_private begin\n",__FUNCTION__);
        msg_queue_put_private(this->messageQueue,&event);
        ...
        if (this->videoq.abort_request && msg_queue_get(this->messageQueue,&event,SDL_ALLOC_EVENT) != 1) {
            while (!vp->allocated) {
                SDL_WaitCond(this->pictq_cond, this->pictq_mutex);
            }
        }
    ...
    if (vp->bmp) {
        ...
        sws_scale(this->img_convert_ctx, src_frame->data, src_frame->linesize, 0, vp->height, pict.data, pict.linesize); ///sws_scale������ͼ�����ź͸�ʽת�����ú�������ʹ�ø��ֲ�ͬ�㷨����ͼ����д���
        */
        ...
        /* workaround SDL PITCH_WORKAROUND workaround����������ͨ������Ȩ���ʩ*/
        ////////duplicate_right_border_pixels(vp->bmp); ///����ʡ��
...
int LJMediaPlayer::get_video_frame(AVFrame *frame, AVPacket *pkt, int *serial) {
    int got_picture, ret;
    ...
    if (pkt->data == this->flush_pkt.data) { ///seek �Żᷢ��?
...
int LJMediaPlayer::synchronize_audio(int nb_samples) {
...
int LJMediaPlayer::audio_decode_frame() {
    ...
    for (;;) {
        ...
        //    SDL_SignalCond(is->continue_read_thread);
...
int LJMediaPlayer::audio_open(int64_t wanted_channel_layout, int wanted_nb_channels,
    ...
    //    return -1;
    //}
    wanted_spec.format = SDL_AUDIO_S16;//AUDIO_S16SYS; ///Audio data format
    ...
    while (SDL_OpenAudio(this->hardware_context,&wanted_spec, &spec) < 0) {
        av_log(NULL, AV_LOG_WARNING, "SDL_OpenAudio (%d channels)\n", wanted_spec.channels);
    ...
    if (spec.format != SDL_AUDIO_S16) { //AUDIO_S16SYS) {
        av_log(NULL, AV_LOG_ERROR,
...
int LJMediaPlayer::check_stream_specifier(AVFormatContext *s, AVStream *st, const char *spec) {
...
AVDictionary* LJMediaPlayer::filter_codec_opts(AVDictionary *opts, enum AVCodecID codec_id,
...
int LJMediaPlayer::stream_component_open(int stream_index) {
    ...
    if(stream_lowres > av_codec_get_max_lowres(codec)){
        ...
        stream_lowres = av_codec_get_max_lowres(codec);//获取解码器最大低分辨率
    }
    ...
    switch (avctx->codec_type) {
        case AVMEDIA_TYPE_AUDIO:
            ...
            ///////////�����߳�
            ///SDL_PauseAudio(is->hardware_context,1);
            break;
        case AVMEDIA_TYPE_VIDEO:
...
void LJMediaPlayer::stream_component_close(int stream_index) {
    AVFormatContext *ic = this->ic;
    AVCodecContext *avctx;
    ...
    switch (avctx->codec_type) {
        case AVMEDIA_TYPE_AUDIO:
            ...
            SDL_CloseAudio(this->hardware_context);
        ...
        case AVMEDIA_TYPE_VIDEO:
            ...
            SDL_WaitThread(this->video_tid, NULL);
        ...
        case AVMEDIA_TYPE_SUBTITLE:
            ...
            SDL_WaitThread(this->subtitle_tid, NULL);
            this->subtitle_tid = 0;
...
int LJMediaPlayer::is_realtime(AVFormatContext *s) {
    if(!s){
        return 0;
    }
...
int LJMediaPlayer::is_seekable(AVFormatContext *s) {
    if(NULL!=s && NULL!=s->pb){
...
AVDictionary** LJMediaPlayer::setup_find_stream_info_opts(AVFormatContext *s,
...
int LJMediaPlayer::videostate_init(AVInputFormat *iformat) {
    ...
    /* start video display */
    this->pictq_mutex = SDL_CreateMutex();
    this->pictq_cond  = SDL_CreateCond();

    this->subpq_mutex = SDL_CreateMutex();
    this->subpq_cond  = SDL_CreateCond();

    packet_queue_init(&this->videoq);
    packet_queue_init(&this->audioq);
    packet_queue_init(&this->subtitleq);

    this->continue_read_thread = SDL_CreateCond();
    ...
    this->mp_listener_lock = SDL_CreateMutex();

    //this->pause_lock = SDL_CreateMutex();;
...
int LJMediaPlayer::do_stream_open() {
    ...
    err = avformat_find_stream_info(ic, opts);  ///modify by SDLh for compile error
    if (err < 0) {
    ...
    if (ret != 0) {
        ...
        event.type = SDL_QUIT_EVENT;//FF_QUIT_EVENT;
        event.data = is; ///event.user.data1 = is;
        msg_queue_put_private(&event);
    }
    */
...
int LJMediaPlayer::stream_start() {
    av_log(NULL, RELEASE_DEBUG_LOG_LEVEL, "%s: into\n",__FUNCTION__);

    /////try here
    if(this->audio_stream >= 0){
        SDL_PauseAudio(this->hardware_context,1);
    }
    if(this->video_stream >= 0){
        this->video_tid = SDL_CreateThread(video_thread, this);
        if(this->video_tid == 0){
    ...
    if(this->subtitle_stream >= 0){
        this->subtitle_tid = SDL_CreateThread(subtitle_thread, this);
...
int LJMediaPlayer::stream_open(AVInputFormat *iformat) {
    //MediaPlayer *is;
    int ret;
...
void LJMediaPlayer::refresh_loop_wait_event(AVMessage *event) {
    double remaining_time = 0.0;
    int ret;
    /*
    while (!msg_queue_get(this->hardware_context,event, SDL_ALLEVENTS)) {
    ...
    do{
        ret = msg_queue_get(this->messageQueue,event, SDL_ALLEVENTS);
        if(1==ret){
            if(event->type==SDL_SEEK_EVENT && this->seeking){
...
int LJMediaPlayer::seek_to(int msec) {
    int64_t ts;
    ...
    for (;;) {
        ...
        switch (event.type) {
            ...
            case SDL_SEEK_EVENT:
                ...
                //	msg_queue_put_private(cur_stream->hardware_context,&event);
                //}else{
                if(!this->seeking){
                ...
                //}
                break;
            case SDL_SET_VOLUME_EVENT:
                msec = (int)event.data;
                av_log(NULL, AV_LOG_DEBUG, "%s: recv a set VOLUME event,volume=%d\n",__FUNCTION__,msec);
                if(NULL!=this->hardware_context && NULL!=this->hardware_context->android_opensles){
                    this->hardware_context->android_opensles->SetSpeakerVolume(msec);
                }
                break;
            default:
                break;
...
// *************** static *******************
int LJMediaPlayer::lockmgr(void **mtx, enum AVLockOp op) {
    switch(op) {
        case AV_LOCK_CREATE:
            *mtx = SDL_CreateMutex();
            if(!*mtx)
...
void* LJMediaPlayer::video_thread(void *arg) {
    AVPacket pkt = { 0 };
    LJMediaPlayer *is = (LJMediaPlayer *)arg;
...
void* LJMediaPlayer::subtitle_thread(void *arg) {
    LJMediaPlayer *is = (LJMediaPlayer *)arg;
    SubPicture *sp;
    AVPacket pkt1, *pkt = &pkt1;
    int got_subtitle;
    ...
    for (;;) {
        ...
        while (is->subpq_size >= SUBPICTURE_QUEUE_SIZE &&
            ...
            SDL_WaitCond(is->subpq_cond, is->subpq_mutex);
        }
...
void LJMediaPlayer::sdl_audio_callback(void *opaque, char *stream, int len) {
    LJMediaPlayer *is = (LJMediaPlayer *)opaque;
    int audio_size, len1;
    int bytes_per_sec;
...
int LJMediaPlayer::decode_interrupt_cb(void *ctx) {
    av_log(NULL, AV_LOG_INFO, "%s: into\n",__FUNCTION__);

    LJMediaPlayer *is = (LJMediaPlayer *)ctx;
    return is->abort_request;
}

void* LJMediaPlayer::read_thread(void *arg) {
    //int eof = 0;
    int err, i, ret;
    AVPacket pkt1, *pkt = &pkt1;
    ...
    for (;;) {
        ...
        if (is->seek_req) {
            if(is->is_seekable(is->ic)){
                ...
                //if (is->paused){ //seek֮���������ʼ����
                //    is->stream_toggle_pause(); //step_to_next_frame(is);
                //    if( is->state == LJPLAYER_STATE_PAUSED ){
                //		is->state = LJPLAYER_STATE_STARTED;
                //	}
                //}
        ...
        if ( ( is->eof ||
            ...
            //(is->subtitleq.nb_packets > is->MAX_FRAMES || is->subtitle_stream < 0 || is->subtitleq.abort_request) ) ) ) { //��ʼʱ��û��ĻBUG : nb_packets=0 subtitle_stream>=0 subtitleq.abort_request=0
            // wait 10 ms
            if(!is->buffer_full){
                if(is->eof){
            ...
            if(!is->eof){
                ...
                SDL_TimeWaitCond(is->continue_read_thread, wait_mutex, 10);
        ...
        if (ret < 0) {
            ...
            if (ret == AVERROR_EOF || url_feof(is->ic->pb)){
                if(ret == AVERROR_EOF){
                    ...
                    */
                    if(is->ic->pb && is->ic->pb->error<0){ //�������߶�����ʱ���ܼ���
    ...
    if (ret != 0) {
        ...
        event.type = SDL_QUIT_EVENT;
        event.data = is;
        msg_queue_put_private(is->messageQueue,&event);
    }
...
void* LJMediaPlayer::event_thread(void *arg) {
    LJMediaPlayer *is = (LJMediaPlayer *)arg;
...
void* LJMediaPlayer::stream_open_thread(void *arg) {
    int ret;
    LJMediaPlayer *is = (LJMediaPlayer *)arg;
    ...
    if (av_lockmgr_register(lockmgr)) {
        ...
        if(is->state==LJPLAYER_STATE_PREPARING){
            goto fail;
        }
        is->stream_open_thread_id = 0;
        return NULL; //return LJPLAYER_ERROR_INTERNAL;
    }
    ...
    if (ret<0) {
        if(is->state==LJPLAYER_STATE_PREPARING){
            av_log(NULL, AV_LOG_ERROR, "%s:failed to open stream, in LJPLAYER_STATE_PREPARING!\n",__FUNCTION__);
            goto fail;
        }
        is->stream_open_thread_id = 0; //-1;
        av_log(NULL, AV_LOG_ERROR, "%s: failed to open stream, exit thread!\n",__FUNCTION__);
        return NULL; //return LJPLAYER_ERROR_NETWORK;
    }
    av_log(NULL, RELEASE_DEBUG_LOG_LEVEL, "%s: stream_open end\n",__FUNCTION__);

    ///event thread, handle event and show video
    //is->event_thread_id = SDL_CreateThread(event_thread, is);

    is->state = LJPLAYER_STATE_PREPARED;
...
void LJMediaPlayer::player_log_callback_help(void *ptr, int level, const char *fmt, va_list vl) {}
