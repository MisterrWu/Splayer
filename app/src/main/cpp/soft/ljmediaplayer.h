...
#ifndef SPLAYER_LJMEDIAPLAYER_H
#define SPLAYER_LJMEDIAPLAYER_H
...
#define	LJPLAYER_ERROR_NORMAL			-1
#define	LJPLAYER_ERROR_PARAMETER 		-2
#define	LJPLAYER_ERROR_NO_MEMORY 		-3
#define	LJPLAYER_ERROR_LJPLAYER_STATE 	-4
#define	LJPLAYER_ERROR_NETWORK 			-5
#define	LJPLAYER_ERROR_UNSUPPORTED 		-6
#define	LJPLAYER_ERROR_TIMEDOUT 			-7
#define	LJPLAYER_ERROR_INTERNAL 			-8

enum ljplayer_state{
    LJPLAYER_STATE_ERROR = -1,
    LJPLAYER_STATE_IDLE = 0,
    LJPLAYER_STATE_INIT ,
    LJPLAYER_STATE_PREPARED ,
    LJPLAYER_STATE_PREPARING ,
    LJPLAYER_STATE_STARTED,
    LJPLAYER_STATE_PAUSED,
    LJPLAYER_STATE_STOPPED,
    LJPLAYER_STATE_COMPLETED,
    LJPLAYER_STATE_END
};

enum video_scaling_mode{
    VIDEO_SCALING_MODE_SCALE_TO_FIT = 0,
    VIDEO_SCALING_MODE_FULL_SCREEN,
    VIDEO_SCALING_MODE_ORIGINAL
};
...
class LJMediaPlayer{
public:
    explicit LJMediaPlayer();
    static LJMediaPlayer* Create();
    static void Destroy(LJMediaPlayer* mp);
    virtual ~LJMediaPlayer();
    virtual int setup(JavaVM* jvm, jobject thiz, jobject weak_this);
    virtual int set_surface(JavaVM* jvm,jobject jsurface);
...
private:
    ...
    void stream_toggle_pause(int unpause); //unpause
    void toggle_pause();
    ...
    void refresh_loop_wait_event(AVMessage *msg);
...
private:
    ...
    double audio_clock;
    double audio_clock_total; //m3u8clock
    ...
    SDL_Thread event_thread_id;

    JavaVM* 	jvm;
    int buffer_full;
    enum ljplayer_state state;
    ...
    int skip_idct;
    int skip_loopfilter;
    int skip_auto_closed;

    SDL_Aout* hardware_context;
    MessageQueue* messageQueue;
    int64_t duration;
    int laset_buffer_percent;
    int MAX_QUEUE_SIZE;
    int MAX_FRAMES;
    LJMediaPlayerListener* mediaplayer_listener;
    SDL_Mutex *mp_listener_lock;
    int eof;
    SDL_Thread stream_open_thread_id;
    int last_notify_msg;
...
#endif //LJPLAYER_LJMEDIAPLAYER_H
